...
export function useChat() {
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    const assistantMsg: ChatMessage = { id: nextId(), role: "assistant", content: "", sources: [] };
    ...
    try {
      await fetchSSE(
        ...
        (srcs) => {
          const typedSources = srcs as Source[];
          setSources(typedSources);
          setMessages((prev) => {
            const updated = [...prev];
            const last = updated[updated.length - 1];
            if (last.role === "assistant") last.sources = typedSources;
            return updated;
          });
        },
        (token) => {
